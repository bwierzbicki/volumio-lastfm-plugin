...
Timer.prototype.start = function (timeOut, callback) {
    ...
    if (!self.timer) {
        ...
        self.isActive = true;
        if (self.debug_mode) console.log('-------------------------------------------------// S T A R T I N G');
    }
...
Timer.prototype.resume = function () {
    ...
    if (!self.timer) {
        ...
        self.timer = setTimeout(self.cb, self.timeRemaining);
        self.isActive = true;
        if (self.debug_mode) console.log('-------------------------------------------------// R E S U M I N G');
    }
    // There is no else, the timer object should always be nullified when pausing/stopping
};
...
Timer.prototype.pause = function () {
    ...
    if (self.timer) {
        ...
        if (self.debug_mode && self.timeRemaining > 0) console.log('-------------------------------------------------// P A U S I N G (remaining: ' + self.timeRemaining + ' milliseconds)');
...
Timer.prototype.stop = function () {
    ...
    if (self.timer) {
        ...
        self.isActive = false;
        self.canContinue = false;
        if (self.debug_mode) console.log('-------------------------------------------------// S T O P P I N G');
    }
    self.timer = null;
};
