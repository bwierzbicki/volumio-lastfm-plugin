...
var compositeTitle = {
    separator: " - ", indexOfArtist: 0, indexOfTitle: 1
}
...
ControllerLastFM.prototype.getUIConfig = function () {
    ...
    self.commandRouter.i18nJson(__dirname + '/i18n/strings_' + lang_code + '.json', __dirname + '/i18n/strings_en.json', __dirname + '/UIConfig.json')
        .then(function (uiconf) {
            ...
            if (self.config.get('password') != undefined && self.config.get('password') != '') uiconf.sections[0].content[3].value = self.config.get('password'); else uiconf.sections[0].content[3].value = '******';
...
ControllerLastFM.prototype.handleBrowseUri = function (curUri) {
    ...
    if (curUri == 'lastfm') {
        ...
        if (curUri == 'lastfm/similar_artist') response = self.getSimilarArtists('similar_artist'); else if (curUri == 'lastfm/similar_tracks') response = self.getSimilarTracks('similar_tracks');
    }
    return response
        .fail(function (e) {
...
ControllerLastFM.prototype.browseRoot = function (uri) {
    var self = this;
    self.fTree = [{label: 'Similar Artists', uri: 'similar_artist', icon: 'fa fa-users'}, {
    ...
    var rootTree = {
        navigation: {
            lists: [{
                availableListViews: ['grid', 'list',], items: [],
            },], prev: {
                uri: '/',
            },
        },
...
ControllerLastFM.prototype.getSimilarTracks = function (uri) {
    ...
    if (self.lfm != null) {
        ...
        call.then(function (response) {
            ...
            if (jsonResp.similartracks.track.length < 1) self.commandRouter.pushToastMessage('info', "No results", "The query yielded no results, no similar track could be found for " + self.scrobbleData.artist + ' - ' + self.scrobbleData.title);
            ...
            for (var trk in jsonResp.similartracks.track) {
                ...
                artwork = self.getAlbumArt({
                    artist: jsonResp.similartracks.track[trk].artist.name,
                    album: jsonResp.similartracks.track[trk].album
                }, undefined, 'fa fa-music');
                rootTree.navigation.lists[0].items.push({
                    service: 'lastfm',
                    type: 'track',
        ...
        .then(function (APIStatus) {
            if (APIStatus) {
                ...
                if (predicate != undefined && predicate.artist != undefined && predicate.title != undefined) {
                    ...
                    if (debugEnabled) self.logger.info('[LastFM] Trying to call api with method ' + method + ' and predicate: ' + JSON.stringify(predicate));
                ...
                } else self.logger.info('[LastFM] Predicate not set, could not populate menu for ' + method);
            } else {
...
ControllerLastFM.prototype.checkURL = function (url) {
    ...
    try {
        ...
        var req = http.request(options, function (r) {
            if (debugEnabled) self.logger.info('[LastFM] URL check (' + url + ') returned code ' + r.statusCode);
            defer.resolve(true);
        });
...
ControllerLastFM.prototype.startScrobbleTimer = function (timerLength, state) {
    ...
    try {
        ...
        self.currentTimer.start(timerLength, function (scrobbler) {
            if (debugEnabled) self.logger.info('[LastFM] scrobbling from restarted timer.');
            self.scrobble();
            self.currentTimer.stop();
            self.timeToPlay = 0;
        });
        if (debugEnabled) self.logger.info('[LastFM] Timer started with time stamp ' + trackStartTime);
        defer.resolve();
    } catch (ex) {
...
ControllerLastFM.prototype.updateCredentials = function (data) {
    ...
    if (data['storePassword'] && data['password'] != undefined && data['password'] != '' && data['password'] != '******') self.config.set('password', data['password']);
...
ControllerLastFM.prototype.updateCompositeTitleSettings = function (titleSeparator, artistFirst) {
    ...
    if (artistFirst) {
        compositeTitle = {
            separator: titleSeparator, indexOfArtist: 0, indexOfTitle: 1
        }
    } else {
        compositeTitle = {
            separator: titleSeparator, indexOfArtist: 1, indexOfTitle: 0
        }
    }
    if (debugEnabled) self.logger.info('[LastFM] Updated composite title settings to: ' + JSON.stringify(compositeTitle));
...
ControllerLastFM.prototype.checkStateUpdate = function (state) {
    ...
    if (blacklistedServices.indexOf(state.service) != -1) {
        if (debugEnabled) self.logger.info(`[LastFM] not processing song, because ${state.service} is blacklisted.`);
    } else {
        if (!self.currentTimer) {
            self.currentTimer = new pTimer(self.context, debugEnabled);
            if (debugEnabled) self.logger.info('[LastFM] created new timer object');
        } else {
            if (debugEnabled) self.logger.info('[LastFM] using existing timer');
        }
        ...
        if (state.stream === true) scrobbleThresholdInMilliseconds = scrobbleThresholdStream; else {
            if ((state.duration != null) && (state.duration > 30)) { // Just to make sure it is always defined!
                ...
                if (debugEnabled) self.logger.info('[LastFM] Undefined track duration or too short for scrobbling: ' + state.duration + ', ' + scrobbleThresholdInMilliseconds);
            }
        }
        ...
        if (debugEnabled) {
            ...
            if (self.currentTimer) self.logger.info('=================> [timer] is active: ' + self.currentTimer.isActive + ' | can continue: ' + self.currentTimer.canContinue + ' | timer started at: ' + self.currentTimer.timerStarted);
        }
    }
    ...
    if (state.status == 'play' && blacklistedServices.indexOf(state.service) == -1) {
        if (debugEnabled) self.logger.info('[LastFM] Playback detected, evaluating parameters for scrobbling...');
        ...
        if (self.previousState.artist == state.artist && self.previousState.title == state.title) {
            ...
            if (self.currentTimer.canContinue && self.timeToPlay > 0) {
                if (debugEnabled) self.logger.info('[LastFM] Continuing scrobbling of paused song, starting new timer for the remainder of ' + self.timeToPlay + ' milliseconds [' + state.artist + ' - ' + state.title + '].');
                self.startScrobbleTimer(self.timeToPlay, state);
            } else if (state.duration != self.previousState.duration) {   // Duration has changed, for the same song. Needed for example for airplay:
                ...
                if (self.currentTimer.isActive) {
                    ...
                    if (scrobbleThresholdInMilliseconds > 0) {
                        ...
                        if (self.formatScrobbleData(state)) // enough metadata to be able to scrobble the track
                        {
                            ...
                            if (debugEnabled) self.logger.info('[LastFM] starting new timer for ' + scrobbleThresholdInMilliseconds + ' milliseconds [' + state.artist + ' - ' + state.title + '].');
                            self.startScrobbleTimer(scrobbleThresholdInMilliseconds, state);
                        }
                    }
                }
            } else {
                if (debugEnabled) self.logger.info('[LastFM] Same state as the one previously pushed. No need to do anything...');
            }
        } else {
        ...
        if (debugEnabled) self.logger.info('[LastFM] Song has been paused, so also pausing timer.');
        if (self.currentTimer.isActive) self.timeToPlay = self.currentTimer.pause();
    } else if (state.status == 'stop') {
        if (self.currentTimer.isActive) {
            if (debugEnabled) self.logger.info('[LastFM] stopping timer, playback has ended.');
            self.currentTimer.stop();
        }
        self.timeToPlay = 0;
    }
...
ControllerLastFM.prototype.initLastFMSession = function () {
    ...
    if ((self.config.get('API_KEY') != '') && (self.config.get('API_SECRET') != '') && (self.config.get('username') != '') && (self.config.get('authToken') != '')) {
        if (debugEnabled) self.logger.info('[LastFM] trying to authenticate...');
        ...
        self.lfm.getSessionKey(function (result) {
            if (result.success) {
                ...
                if (debugEnabled) self.logger.info('[LastFM] authenticated successfully!');
        ...
        if (self.config.get('API_KEY') == '') msg += '  "API_KEY"';
        if (self.config.get('API_SECRET') == '') msg += '  "API_SECRET"';
        if (self.config.get('username') == '') msg += '  "username"';
        if (self.config.get('authToken') == '') msg += '  "authToken"';
        self.commandRouter.pushToastMessage('error', 'LastFM connection failed', msg);
...
ControllerLastFM.prototype.updateNowPlaying = function () {
    var self = this;
    var defer = libQ.defer();
    if (debugEnabled) self.logger.info('[LastFM] Updating now playing');
    ...
    if (self.scrobblableTrack && self.lfm != null) {
        ...
        self.lfm.getTrackInfo({
            ...
            callback: function (result) {
                ...
                } else self.logger.error('[LastFM] track info request failed with error: ' + result.error);
            }
        });
        ...
        self.lfm.scrobbleNowPlayingTrack({
            ...
            callback: function (result) {
                if (!result.success) console.log('[LastFM] updated "now playing" failed: ', result); else {
                    if (debugEnabled) self.logger.info('[LastFM] updated "now playing" | artist: ' + self.scrobbleData.artist + ' | title: ' + self.scrobbleData.title);
                }
                self.updatingNowPlaying = false;
            }
        });
...
ControllerLastFM.prototype.scrobble = function () {
    ...
    if (self.scrobblableTrack && self.lfm != null) {
        if (debugEnabled) self.logger.info('[LastFM] preparing to scrobble...');
        ...
        self.lfm.scrobbleTrack({
            ...
            callback: function (result) {
                if (result.success) {
                    if (self.scrobbleData.album == undefined || self.scrobbleData.album == '') self.scrobbleData.album = '[unknown album]';
                    if (self.config.get('pushToastOnScrobble')) self.commandRouter.pushToastMessage('success', 'Scrobble succesful', 'Scrobbled: ' + self.scrobbleData.artist + ' - ' + self.scrobbleData.title + ' (' + self.scrobbleData.album + ').');
                    if (debugEnabled) self.logger.info('[LastFM] Scrobble successful for: ' + self.scrobbleData.artist + ' - ' + self.scrobbleData.title + ' (' + self.scrobbleData.album + ').');
                } else {
                    console.log("in callback, finished: ", result);
                    if (self.config.get('pushToastOnScrobble')) self.commandRouter.pushToastMessage('error', 'Scrobble failed', 'Tried to scrobbled: ' + self.scrobbleData.artist + ' - ' + self.scrobbleData.title + ' (' + self.scrobbleData.album + ').');
                    if (debugEnabled) self.logger.info('[LastFM] Scrobble failed for: ' + self.scrobbleData.artist + ' - ' + self.scrobbleData.title + ' (' + self.scrobbleData.album + ').');
                }
            }
        });
